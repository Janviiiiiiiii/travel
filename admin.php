...
<body>
	...
	<div class="container">
		...
		<div class="menu-list">
			...
			<a id="a2" href="#" onclick="myFunction(document.getElementById(this.id))">Bookings</a>
			...
			<a id="a4" href="#" onclick="myFunction(document.getElementById(this.id))">Delete Place Information</a>
			<a id="a7" href="#" onclick="myFunction(document.getElementById(this.id))">Feedback</a>
		...
		<div class="agent-workspace work" id="id2">
			<div class="btn-tag" id="agn-op">
				<button type="button" id="v2" onclick="showDetails(document.getElementById(this.id))">View Booking Detailes</button>
				
				<button type="button" id="b2" onclick="deleteMenu(document.getElementById(this.id))">Delete Booking</button>
			</div>
			<div id="agent-container" style="display: none; margin-top: 50px;">
				<table align="center" border="1px" style="width: 700px; line-height: 30px;">
					<tr>
						<th colspan="7"><h2>Booking Details</h2></th>
					</tr>
					<tr>
						<th>Id</th>
						<th>Name</th>
					...
					<tr>
						...
						<td><?php echo $rows1['ffirst']; ?></td>
						<td><?php echo $rows1['femail']; ?></td>
						<td><?php echo $rows1['city']; ?></td>
						<td><?php echo $rows1['fphone']; ?></td>
						<td><?php echo $rows1['fdesti']; ?></td>
			...
			<div class="delete-form" id="del-form2">
				<form method="POST" action="admin_op.php">     
					...
					<input type="text" name="ffirst" placeholder="First name" required><br>
		...
		<div class="places-workspace work" id="id3">
			...
			<div id="place-container" style="display: none; margin-top: 50px;">
				<table align="center" border="1px" style="width: 500px; line-height: 30px;">
					...
					?>
					<tr>
						<td><?php echo $rows2['sno']; ?></td>
						<td><?php echo $rows2['pname']; ?></td>
						<td><?php echo $rows2['package']; ?></td>
					</tr>
					<?php
						}
					?>
		...
		<div class="hotels-workspace work" id="id4">
			<div class="btn-tag" id="htl-op">
				<button type="button" id="b4" onclick="deleteMenu(document.getElementById(this.id))">Delete Place Information</button>
			</div>
			<div class="delete-form" id="del-form4">
				<form method="POST" action="admin_op.php">     
					<input type="text" name="sno" placeholder="Palce ID" required><br>
		...
		<div class="addp-workspace work" id="id5" style="display: none;">
			<div class="insert-pform" id="insp-form4">
				<form method="POST" action="admin_op.php">
					...
					<input type="text" name="package" placeholder="Enter package price" required><br>
					<input type="submit" value="Add Place Details" class="submit" name="add_pd">
				</form>
			</div>
		</div>

		<div class="fdk-workspace work" id="id7">
			<div class="btn-tag" id="fdk-op">
				<button type="button" id="v7" onclick="showDetails(document.getElementById(this.id))">View Feedback</button>
				</div>
			<div id="fdk-container" style="display: none; margin-top: 50px;">
				<table align="center" border="1px" style="width: 500px; line-height: 30px;">
					<tr>
	...
	<script type="text/javascript">
		function makeNone(){
				document.getElementById("fdk-container").style.display = "none";
		...
		function myFunction(clicked){
			...
			if (document.getElementById('a7') === clicked){
				...
				document.getElementById("fdk-op").style.display = "block";
				document.getElementById("del-form4").style.display = "none";
			}
		}
		...
		function showDetails(clicked){
			...
			if (document.getElementById('v7') === clicked){
				document.getElementById("fdk-op").style.display = "none";
				document.getElementById("del-form4").style.display = "none";
				document.getElementById("fdk-container").style.display = "block";
			}
